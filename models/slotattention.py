...
class SlotAttentionAutoencoder(Autoencoder):
    ...
    def setup(self):
        ...
        self.slot_attention = SlotAttentionModule(
            self.num_slots, self.slot_size, self.iters, self.mlp_hidden_size, self.decoder_init_shape)
        self.decoder = DecoderCNN(num_channels=self.output_shape[-1])

        self.encoder_pos = SoftPositionEmbed(
            self.slot_size, (self.output_shape[1], self.output_shape[1]))
        self.decoder_pos = SoftPositionEmbed(
            self.slot_size, self.decoder_init_shape)

        self.mlp = nn.Sequential(
            [nn.Dense(self.slot_size), nn.relu, nn.Dense(self.slot_size)])
    ...
    def decode(self, slots, batch_size=1):
        ...
        block_recons = reshape_recons(
            recons, batch_size, self.output_shape[-1])
...
def reshape_recons(recons: jnp.ndarray, batch_size: int, num_channels=3):
    block_recons = jnp.reshape(
        recons, [batch_size, -1] + list(recons.shape)[1:])
...
class EncoderCNN(nn.Module):
    ...
    def __call__(self, x):
        ...
        print(x.shape)
        x = nn.Conv(features=self.num_features,
                    kernel_size=(5, 5), padding='SAME')(x)
        x = nn.relu(x)
...
class SlotAttentionModule(nn.Module):
    ...
    def __call__(self, inputs):
        ...
        slots_mu = self.param('slots_mu', nn.initializers.xavier_uniform(
        ), (1, self.num_slots, self.slot_size))
        slots_log_sigma = self.param('slots_log_sigma', nn.initializers.xavier_uniform(
        ), (1, self.num_slots, self.slot_size))
        # Initialize slots
        slots = slots_mu + \
            jnp.exp(slots_log_sigma) * random.normal(rng,
                                                     (inputs.shape[0], self.num_slots, self.slot_size))
        print(inputs.shape)
        print(self.slot_size)
        k, v = nn.Dense(self.slot_size, self.slot_size)(
            inputs), nn.Dense(self.slot_size, self.slot_size)(inputs)
        ...
        slots = jnp.tile(
            slots, [1, self.decoder_init_shape[0], self.decoder_init_shape[0], 1])
...
class DecoderCNN(nn.Module):
    ...
    def __call__(self, x):
        ...
        x = nn.ConvTranspose(features=self.num_channels, kernel_size=(
