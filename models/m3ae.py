...
class CLIPModel(VisionLanguageModel):
    ...
    def imtext2feature(self, image: Tensor, text) -> Tensor:
        if isinstance(text, str):
            ...
            text = torch.from_numpy(text["input_ids"][0])[None, ...]

        cls_x, image_x, text_x, image_mask, text_mask, image_ids_restore, text_ids_restore = self.model.forward_encoder(image, text)

        combined_features = torch.cat([image_x, text_x], dim=1)
        
        return combined_features

    # def im2feature(self, image: Tensor) -> Tensor:
    #     return self.model.forward_encoder(image, "")
