...
class SlotAttention(nn.Module):
    ...
    def forward(self, inputs, num_slots=None):
        ...
        mu = self.slots_mu.expand(b, n_s, -1)
        sigma = self.slots_sigma.expand(b, n_s, -1)
        slots = torch.normal(mu, sigma)  # torch.zeros(mu.shape).to(device)  #
...
class SlotAttentionCompressionAutoencoder(nn.Module):
    ...
    def forward(self, x, upto: int = None):
        ...
        # TODO: add conditioning on slots
...
class SlotAttentionCompressionAutoencoderDirect(nn.Module):
    ...
    def forward_step(self, z, step: int):
        if step == 0:
            ...
            x_hat, _, _, _ = self.slot_attention_autoencoder.decode(
                z_km1, z.shape[0])
            return x_hat
...
class SlotAttentionCompressionDiffusion(nn.Module):
    def __init__(self,
                 autoencoder: SlotAttentionCompressionAutoencoderDirect,
                 hid_dim: int = 256,
                 frequency_embedding_size=64):
        super().__init__()
        self.autoencoder = autoencoder
        self.time_embedding = nn.Sequential(
            nn.Linear(frequency_embedding_size, hid_dim),
            nn.SiLU(),
            nn.Linear(hid_dim, hid_dim),
        )

    def forward(self, x, y, t, upto: int):
        z = self.slot_attention_autoencoder.encode(x)
        # TODO: add conditioning on slots
        z = self.slot_attention_autoencoder.slot_att(z)
        ...
        # x = x + t_emb + y_emb
        x_recon = self.autoencoder.forward_step(x)
        return x_recon

    # For training diffusion
    def forward_step(self, x, y, t, upto: int):
        print("HERE IN FORWARD STEP")
        # encode image upto some layer
        z = self.slot_attention_autoencoder.encode(x)
        # TODO: add conditioning on slots
        z = self.slot_attention_autoencoder.slot_att(z)
